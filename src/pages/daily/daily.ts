...
export class DailyPage {
  ...
  getEnrollments() {
    this.classroomService.getEnrollments(this.professorId).then((result) => {
      result['enrollments'].forEach(element => {
        console.log(element['enrollment']['student']);
        this.enrollments.push({name: element['enrollment']['student']['name'], gender: element['enrollment']['student']['gender'], avatar: 'assets/imgs/placeholder.png'})
      });
